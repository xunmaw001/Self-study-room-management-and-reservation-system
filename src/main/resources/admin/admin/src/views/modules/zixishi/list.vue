        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '自习室标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.zixishiName" placeholder="自习室标题" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '自习室类型' : ''">
                        <el-select v-model="searchForm.zixishiTypes" placeholder="请选择自习室类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in zixishiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zixishi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/zxsguanliyuyue/upload/zixishiMuBan.xls"
                        >批量导入自习室信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zixishi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zixishiUploadSuccess"
                                :on-error="zixishiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('zixishi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入自习室信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zixishi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zixishi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            {{scope.row.zixishiName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="zixishiPhoto"
                                     header-align="center"
                                     width="200"
                                     label="自习室照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.zixishiPhoto">
                                <img :src="$base.url+scope.row.zixishiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('zixishi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zixishi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('zixishi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zixishi',//当前表表名,
                        sumColum : 'zixishi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zixishi_name',//分组字符串字段
                        // types : 'zixishi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.zixishiName!= '' && this.searchForm.zixishiName!= undefined) {
                    params['zixishiName'] = '%' + this.searchForm.zixishiName + '%'
                }
                 
                if (this.searchForm.zixishiTypes!= '' && this.searchForm.zixishiTypes!= undefined) {
                    params['zixishiTypes'] = this.searchForm.zixishiTypes
                }
                                                
                params['zixishiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=zixishi_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.zixishiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            zixishiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zixishi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
